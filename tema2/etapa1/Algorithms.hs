...
search f node graph = search_helper f node graph [node] [node]

search_helper f node graph visited stack_queue
    | stack_queue == [] = visited
    | (head stack_queue) `elem` visited = search_helper f (head stack_queue) graph visited (f (tail stack_queue) (listDiff visited (S.toList (outNeighbors (head stack_queue) graph))))
    | otherwise = search_helper f (head stack_queue) graph (visited ++ [head stack_queue]) (f (tail stack_queue) (listDiff visited (S.toList (outNeighbors (head stack_queue) graph))))


listDiff a b = filter (\x -> not (x `elem` a)) b
{-
    *** TODO ***
...
countIntermediate from to graph = if (elem to (bfs from graph)) then Just (len, len') else Nothing
    where
    len = length (fst (span (/= to) (snd (span (/= from) (bfs from graph))))) - 1
    len' = length (fst (span (/= to) (snd (span (/= from) (dfs from graph))))) - 1
