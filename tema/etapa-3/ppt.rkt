...
(define (get-transformations n)
  (get-transformations-helper n (get-level n) (get-a (get-level n)) (get-b (get-level n)) '()))

(define (get-transformations-helper n level a b Result)
  (cond ((zero? level) Result)
        ((< n a) (get-transformations-helper n (- level 1) (+ (- a (expt 3 (- level 1))) (expt 3 (- level 2))) (- (- a 1) (expt 3 (- level 2))) (append Result '(1))))
        ((> n b) (get-transformations-helper n (- level 1) (+ (+ b 1) (expt 3 (- level 2))) (- (+ b (expt 3 (- level 1))) (expt 3 (- level 2))) (append Result '(3))))
        (else (get-transformations-helper n (- level 1)  ( + a (expt 3 (- level 2))) (- b (expt 3 (- level 2))) (append Result '(2))))))

(define (get-level n)
  (get-level-helper n 1 0))

(define (get-level-helper n sum level)
  (if (>= sum n)
      level
      (get-level-helper n (+ sum (expt 3 (+ level 1))) (+ level 1))))

(define (get-a level)
  (+ (+ (/ (- (expt 3 level) 1) 2) 1) (expt 3 (- level 1))))

(define (get-b level)
  (- (+ (get-a level) (expt 3 (- level 1))) 1))
...
(define (Tr1 a b c) (multiply T1 (list a b c)))
(define (Tr2 a b c) (multiply T2 (list a b c)))
(define (Tr3 a b c) (multiply T3 (list a b c)))

(define get-nth-ppt-from-matrix-transformations
  (get-nth-tuple Tr1 Tr2 Tr3 '(3 4 5)))
...
(define get-nth-quadruple
  (get-nth-tuple Q1 Q2 Q3 '(1 1 2 3)))
