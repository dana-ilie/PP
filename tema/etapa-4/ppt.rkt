...
(define ppt-stream-in-tree-order
  (let loop ((Q (stream-append empty-stream (list '(3 4 5)))))
    (stream-cons (stream-first Q)
                (loop (stream-append (stream-rest Q)
                                      (list (multiply T1 (stream->list (stream-first Q))))
                                      (list (multiply T2 (stream->list (stream-first Q))))
                                      (list (multiply T3 (stream->list (stream-first Q)))))))))
...
(define (pairs G H)
  (let loop ((A G) (B H) (count 0) (nr-pairs 1))
    (cond ((= count nr-pairs) (loop G (stream-rest B) 0 (+ 1 nr-pairs)))
          (else (stream-cons (cons (stream-first A) (stream-first B))
                             (loop (stream-rest A) B (+ 1 count) nr-pairs))))))
...
(define (make-stream k)
  (stream-cons k (make-stream (+ 2 k))))

(define G-stream (make-stream 1))
(define H-stream (make-stream 3))

(define gh-pairs-stream
  (stream-filter (λ (p) (= 1 (gcd (car p) (cdr p)))) (pairs G-stream H-stream)))

; TODO
; Definiți fluxul de TPP corespunzător fluxului anterior de
; perechi (g, h).
(define ppt-stream-in-pair-order
  (let loop ((stream gh-pairs-stream))
    (let ((g (car (stream-first stream))) (h (cdr (stream-first stream))))
      (let ((a (* g h)) (b (/ (* (- h g) (+ h g)) 2)) (c (/ (+ (expt h 2) (expt g 2)) 2)))
        (stream-cons (list a b c ) (loop (stream-rest stream)))))))
